...
| [src/youtube.js](src/youtube.js) | YouTube link detection (`findYouTubeUrl`/`parseVideoId`) + transcript fetch: a zero-dep HTTP path (watch page → `captionTracks` → timedtext `json3`), with a `yt-dlp` fallback (if installed; `TLDR_YTDLP=0` disables it) for when YouTube bot-gates the direct fetch. That fallback asks for a **literal** `en,en-orig,en-US,en-GB` first and only widens to `en.*,en` when it finds nothing (`subLangsFor`): `--sub-langs` entries are **case-insensitive full-match regexes**, so `en.*` matches ~55 tracks (53 of them auto-translated `en-<lang>` junk) and the download storm earns a `429`. Never make the wide pattern the first attempt, and don't try to narrow it with a regex like `en-[A-Z]{2}` (the case-insensitive match catches `en-sq` too). The one place to re-probe if YouTube changes and auto-TLDR stops working. |
