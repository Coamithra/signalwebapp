...
export const INSTALL_SCRIPT = `(function () {
  ...
  // Our own conversation id. Reactions identify their author by conversationId,
  // so this is what "did I react" compares against.
  function ourConversationId() {
    try { return (window.reduxStore.getState().user || {}).ourConversationId || null; }
    catch (_) { return null; }
  }
  ...
  function formatMessage(m) {
    ...
    // as an empty pill. r.fromId is a conversationId (not a serviceId), which is
    // both why resolveAuthorTitle works on it and how we spot our own.
    var ourId = ourConversationId();
    var reactions = Array.isArray(m.reactions) ? m.reactions.filter(function (r) {
      return r && r.emoji;
    }).map(function (r) {
      return { emoji: r.emoji, from: resolveAuthorTitle(r.fromId), fromMe: !!ourId && r.fromId === ourId };
    }) : [];
